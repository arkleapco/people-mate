...
def CreateWorkingPolicyView(request):
    ...
    user_lang = to_locale(get_language())
    try:
        working_days_policy= Working_Days_Policy.objects.get(enterprise=request.user.company)
    except Exception as e:
        if request.method == 'POST':
        ...
        error_msg = 'Your company already have policy'
        messages.error(request,error_msg)
        return redirect('company:policy-correct',pk=working_days_policy.id)
...
def update_working_hours_deductions_view(request, deduction_id):
    deduction = Working_Hours_Deductions_Policy.objects.get(id= deduction_id)
    working_deductions_form = WorkingHoursDeductionForm(instance=deduction)

    if request.method == 'POST':
        company_working_policy = Working_Days_Policy.objects.get(enterprise=request.user.company)
        working_deductions_form = WorkingHoursDeductionForm(request.POST,instance=deduction)
        if working_deductions_form.is_valid():
            obj = working_deductions_form.save(commit=False)
            obj.working_days_policy = company_working_policy
            obj.created_by = request.user
            obj.last_update_by = request.user
            obj.last_update_date = date.today()
            obj.save()
            messages.success(request, _('Working Hours Deductions Created Successfully'))
            return redirect('company:working-hrs-deductions-list')

        else:
            messages.error(request, working_deductions_form.errors)

    context = {
        "page_title": _("Work Hours Deduction Policy"),
        'working_deductions_form': working_deductions_form,
     }
    return render(request, 'working-hrs-deductions-update.html', context=context)



@login_required(login_url='home:user-login')
def delete_working_hours_deductions(request, deduction_id):
    working_deduction = Working_Hours_Deductions_Policy.objects.get(id=deduction_id)
    deleted = working_deduction.delete()
    if deleted:
        messages.success(request, 'Record successfully deleted')
...
def send_invoice(request):
    obj = Send_Invoice(request.user,2,2022)
